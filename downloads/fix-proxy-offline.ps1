# Fix ReProxy offline / Proxy.exe killed by Defender
$ErrorActionPreference = "Continue"

$Dir = Join-Path $env:LOCALAPPDATA "ReProxy"
$Exe = Join-Path $Dir "Proxy.exe"
$Log = Join-Path $Dir "proxy-agent.log"
$InstallLog = Join-Path $env:TEMP "ReProxy-install.log"
...
function Add-DefenderExclusion {
    ...
    try {
        ...
        Write-Step "defender process ok: Proxy.exe"
    } catch {
...
function Test-ProxyRunning {
    return [bool](Get-Process -Name "Proxy" -ErrorAction SilentlyContinue)
}

Write-Step "ReProxy fix start"
if (-not (Test-IsAdmin)) {
    Write-Host ""
    Write-Host "Tip: run PowerShell as Administrator so Defender exclusions apply." -ForegroundColor Yellow
    Write-Host "If Defender already quarantined Proxy.exe: Windows Security -> Protection history -> Restore/Allow." -ForegroundColor Cyan
    Write-Host ""
}

Write-Step "Stopping Proxy..."
Get-Process -Name "Proxy" -ErrorAction SilentlyContinue | Stop-Process -Force -ErrorAction SilentlyContinue
Start-Sleep -Seconds 2

New-Item -ItemType Directory -Force -Path $Dir | Out-Null
Add-DefenderExclusion -Path $Dir -ExePath $Exe

$installScript = Join-Path $env:TEMP "ReProxy-install.ps1"
$installUrl = $BaseUrl + "/ReProxy-Install.ps1"
Write-Step ("Downloading " + $installUrl)
if (-not (Download-File -Url $installUrl -OutFile $installScript)) {
    Write-Host ("download ReProxy-Install.ps1 failed: " + $installUrl) -ForegroundColor Red
    exit 1
}
...
Write-Step "Running ReProxy-Install.ps1..."
...
if (-not (Test-ProxyRunning) -and (Test-Path -LiteralPath $Exe)) {
    Write-Step "Proxy not running, starting again..."
    try {
...
if (Test-ProxyRunning) {
    Write-Host ""
    Write-Host "Proxy.exe is running." -ForegroundColor Green
} else {
    Write-Host ""
    Write-Host "Proxy.exe still not running. Defender may have blocked it again." -ForegroundColor Red
    Write-Host "Open Windows Security -> Protection history, restore Proxy.exe, then rerun this script as Administrator." -ForegroundColor Cyan
}
...
if (Test-Path -LiteralPath $Log) {
    ...
    Write-Host "Proxy.exe installed. proxy-agent.log will appear after first connect."
}
